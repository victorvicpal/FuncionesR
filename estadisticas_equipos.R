estadisticas_equipos <- function(season)
{
	m <- dbDriver("MySQL")
	con <- dbConnect(m,user='root',host='localhost',dbname='nba')
	sql <- paste("select distinct hometeam from nba.game where hometeam not in ('ALL','TAR') and season='",season,"'",sep="")
	teams <- dbGetQuery(con,sql)
	sql2 <- paste("select min(game_id) from nba.play_by_play a, nba.game b where a.game_id=b.id and b.season='",season,"'",sep="")
	min <- dbGetQuery(con,sql2)
	sql3 <- paste("select max(game_id) from nba.play_by_play a, nba.game b where a.game_id=b.id and b.season='",season,"'",sep="")
	max <- dbGetQuery(con,sql3)
	...
	for (i in 1:1293)
	{
		...
		sql5 <- paste("select * from nba.play_by_play where game_id=",i,sep="")
		prematch <- dbGetQuery(con,sql5)
		match <- prematch[,13:ncol(prematch)]
